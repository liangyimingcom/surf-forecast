...
- [x] 🔒 G1-b(第一步) 四点已标 `op_status=pending` 移出推荐池（2026-08-05，已实测生效：
      国外池 7→5、总池 37→35 全新鲜；目录页显「待开放」角标）
- [ ] 🔒 G1-b(第二步) **坐标修正** —— 调研结论如下，两点已有权威坐标可直接写：
  - [ ] `sl84 Kirra` → **-28.1655, 153.5280**（Kirra Point, 黄金海岸。依据：快照 `city="澳洲黄金海岸"`
        + Wikipedia Kirra Hill `28.1681°S 153.5333°E`；实测 WAM 格点 `-28.0,153.5` 24/24 有数据 1.06m/Tm8.2s）
  - [ ] `sl85 Currumbin` → **-28.12361, 153.48028**（Currumbin Alley，Wikipedia/Wikidata Q5195186；
        快照 `city="澳洲黄金海岸"` 印证；同格点有数据）
  - [ ] `sl71 海螺湾` → **查无权威坐标，不宜推断**：上游快照自身 `city="（待开放）"`、无 `live_src`，
        `region_cn=浙江` 与坐标（广西/广东内陆）矛盾，经度是借 `sl57` 的 → 未开放浪点的占位行。
        处置：保持 pending，等上游/站长给出真实坐标
  - [ ] `sl97 SURFPARK` → **坐标很可能是对的，问题不在坐标**：`39.894,116.598` = 北京（内陆），
        名为 SURFPARK 且自带 `surfpark.m3u8` 直播 = **人工浪池**；WAM 该格点 24 时点全空属必然。
        处置：需要一个「非海洋浪点」分类（排除出海洋刷新池、保留直播），而不是改坐标
  - [ ] 旧值备份：`docs/ops-backup/registry-sl8{4,5}-before-20260805-pending.json`
- [ ] 🔒 G1-b(原始说明，保留供对照) 四点坐标串行 —— 生产 DynamoDB 写：
