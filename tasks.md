...
- [x] S5.1 `pytest -q` = **329**，与基线一致（后端零改动的硬证据）
- [x] S5.2 `vue_spa.mjs` **32 → 61 全绿 + 0 JS 报错**；新块均有断言：
      走势 · 倒计时(假时钟双侧边界) · 通勤倒推(含仅小白模式的反向断言) ·
      罗盘(逐支箭比对 windKind) · 单位切换(切 ft 后轴上无米制残留 + 持久化) · 夜读持久化
- [x] S5.3 三屏截图与原型逐屏对照（独立子代理做块级对照 + 令牌/死代码审计）→ 折回 4 项：
      ① 🔴 **首页缺最佳窗口倒计时**（原型 `vHome` 有 `cdownEl`，且 `countdown.js` 与走势那次
         report 都已就位）→ 已补，**复用走势同一份数据，零新增请求**，严格按 `best.day` 日期对齐
      ② 🔴 `SpotPage` 选中日 halo 写死 `rgba(14,165,233,.2)`，**夜读不跟随**（真缺陷）→ 新增
         `--halo` 令牌（日/夜两值），实测已随夜读切换
      ③ 🔴 `StatusPage` 阴影与 `--shadow` 重复 → 复用令牌；`App.vue` 遮罩 → 新增 `--overlay`
      ④ 令牌纪律现在**只剩 `LiveCam` 的 `#000`**（视频信箱底，产品例外，已注明）
      ⑤ 审计的一处分类需更正：「首页横评缺浪高/晨风两列」应为 🟡 而非 🔴 ——
         后端 `alternatives` 只给 `spot_slug/spot_name/day/week/score`，**没有** `hs`/`dawnWind`
- [x] S5.4 **零上下文可用性评审**（独立子代理，仅看三张截图，不读任何本轮文档）→ 已折回 3 项：
      ① 🔴 后端 `_key_factors` 把 `dawnWind` **原始枚举 off** 直接示人，评审读成「关闭/没风」，
         而它其实是**最好**的风况（离岸=梳面）→ 显示层翻译，标签复用 `charts.WIND_META`
      ② 🔴 `Tp9s` 术语裸奔 → 「峰周期 9s」
      ③ 🔴「窗口 18:00-21:00**到**」vs 通勤卡「17:00 出门 → 18:00 下水」语义冲突
         （评审：按「到」理解会整整迟到一小时）→ 详情页明写「🕐 **下水窗口** 18:00-21:00」
      ④ 可发现性：夜读/登录图标补 `title`，通勤 ±  补 `title`（评审说易被当静态文字）
...
- [ ] `charts.lifecycleChart(days)` **无生产调用者**（从单 HTML 版移植时带过来的）。
      本轮未删：它不是本轮引入，删除属独立清理，且要先确认没有别的分支在等它。
- [ ] 小白页「📤 分享」与「📅 加入日历(.ics)」——原型有、纯前端可做，但**不在本轮 ✅ 档清单**内。
- [ ] 首页横评补「浪高/晨风」两列 → 需扩 `/api/recommend` 的 `alternatives`（后端改动）。
