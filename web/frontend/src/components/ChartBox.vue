...
<style scoped>
.chartbox { background: var(--card); border-radius: 14px; padding: 10px; margin: 10px 0; }
.chart :deep(svg) { width: 100%; height: auto; display: block; }
.chart :deep(.windq) { display: flex; gap: 2px; }
.chart :deep(.seg) { flex: 1; display: flex; flex-direction: column; align-items: center; padding: 4px 0; border-radius: 5px; color: var(--card); }
.chart :deep(.wq-ar) { font-size: 13px; }
.chart :deep(.wq-k) { font-size: 11px; font-weight: 700; }
.chart :deep(.wq-h) { font-size: 8px; opacity: .9; }
.chart :deep(.windq-key) { display: flex; flex-wrap: wrap; gap: 8px; font-size: 10px; margin-top: 6px; color: var(--ink2); }
