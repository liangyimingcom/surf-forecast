...
// ✅ 2026-08-06 已在后端根治（`src/web/recommend.py:_DAWN_CN` + 「峰周期」措辞）。
// 下面这层翻译**保留作兜底**，只在两种情况下还会命中：① 生产尚未部署后端修复时；
// ② 将来某个客户端/旧缓存路径又把枚举原样透出。认不出的 chip 一律原样放行，
// 所以后端出人话之后这里是幂等的（不会二次翻译）。
// —— 原始问题（留作病历）——
// 后端 `_key_factors` 曾直接把 `dawnWind` 的**原始枚举**（off/cross/on）与 `Tp{n}s` 塞进 chips。
