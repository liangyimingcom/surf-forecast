...
## 已有基础（本轮起点，勿重复）
已完成两轮 UI 优化：**第一轮**(3标签页/吸顶/回顶 · 目录/直播子视图 · hero 动态浪点名 · 目录排序 · 深色模式 · 加载骨架屏 · tab记忆 · 空态)；**第二轮**(布局重定位 #spotbar/#liveEntry · U-a 目录★收藏 · U-b 地图收藏着色 · U-c 可达性 · U-d chips横滑 · U-e 分享深链)。后端：get_report 引擎自算 + /api/catalog + /api/cams + /api/spots(全 401)。**pytest 145、E2E 56/56** 为当前基线。第三轮从 north_star 方向或 V 候选挑最高杠杆。
