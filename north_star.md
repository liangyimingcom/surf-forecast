...
## 已有基础（本轮起点，勿重复）
前端已含：3 主标签页 + 吸顶导航 + 回顶 + hero 动态浪点名(#spotbar 移到标签栏下) + 目录/直播子视图 + 目录排序(5档) + 深色模式 + 加载骨架屏/spinner + tab·子视图记忆(sf_tab_v1/sf_liveview_v1) + 空态提示 + 直播入口(#liveEntry 移到日期条下) + 收藏/搜索/地图 + 58 浪点目录(区域chips+评分徽标) + hls.js 直播 + 公告/反馈/关于/活动墙/拼车(示例)。后端：get_report 引擎自算 + /api/catalog + /api/cams + /api/spots(全 401)。**pytest 145、E2E 47/47** 为当前基线。
