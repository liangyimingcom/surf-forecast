...
- 2026-08-05 17:43 GMT+8 · (生产数据变更·无部署) · `58c02ad` · **sl75/sl76 坐标修正** — 源快照 `lat=110.363232`(>90 非法) 致 Open-Meteo 每日报错、刷新固定 58/60 一周。按同 beach_group 兄弟点推断坐标写入：sl75 石梅湾九里 `18.652,110.279`(兄弟 sl57/sl91)、sl76 富力湾全景 `18.532,110.112`(兄弟 sl39)；同步重算 `dedup_key`，加 `coord_source` 标注坐标为推断值；新值经全表 4dp 碰撞检查（无碰撞，避免 v0.3.2 那类 `find_registry_by_coord` 解析歧义）。旧值备份 `docs/ops-backup/registry-sl7{5,6}-before-20260805.json`。触发 `refresh_cli retry` → **manifest 60/60、failed=[]**，两点详情报告 6 日数据正常（`calibratedAt 17:43 GMT+8`）。
