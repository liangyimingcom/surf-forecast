...
| ADR | 决策 |
|-----|------|
| ADR-5 | LLM 进**在线匿名澄清**(ECS 调 <llm-gateway>/sonnet-4-6)。**连通性已实测**(生产私有子网+NAT → PROBE_REACHABLE http 401)，可达 |
...
### L2 新接口
- 后端 `POST /api/clarify`（匿名+限流）：入参 {page, 步骤, 已选/自由文本} → 出参 {下一步选项菜单 | 收敛为结构化需求}。内部：查选项缓存→未命中调 <llm-gateway>(key from Secrets)→schema 校验→写缓存；预算/限流/报错→降级模板。
