...
## 2026-08-05 · S1 晨报纸感设计令牌 + 夜读模式（v4 三屏 loop cycle 1）

- **S0**：分支 `feat/v4-three-screen-ui`（off master `0a4db78`）；基线 pytest 329 · vue_spa 32/0。
  基准图沿用 `docs/screenshots/43-design-v4-update1-3screens.png`。
- **S1.1 令牌**：`style.css` 5 行 → **29 个令牌**。11 个纸墨海的值**逐字取自原型 `.dirA`/`.dirA.night`**
  （原型自己就用 CSS 变量且备了两套值，等于规格直接可搬）；另加 6 个状态色（/status、直播、
  会员角标、校验——原型没有这些面）和 12 个图表色（取自原型 `T()`）。
- **S1.2 夜读**：`body.night` **只覆盖变量**，不碰任何组件规则；localStorage 键沿用单 HTML 版
  `sf_night_v1`；无偏好时跟随系统深色（surfer 睡前查浪报的真实场景）。
- **S1.3 令牌化 126 处**：四页 69 + 三组件 19 + App.vue + **`charts.js` 51 处**。
  charts.js 是本阶段最关键的一块——它原本写死 sky/emerald/amber 系，夜读一切换就露馅；
  改成 `var(--ch-*)`（内联 SVG 能继承 CSS 变量）。映射用**可审映射表**逐项归并，
  不做盲替换；长色值优先匹配，避免 `#fff` 抢先吃掉 `#fff7ed`。
  唯一保留：`LiveCam` 的 `#000`（视频信箱底，两态都该黑）。
- **验收**：`getComputedStyle` 抽查确认取的是变量而非写死色（夜态链接 `rgb(124,196,224)`=`--sea` 夜值）；
  日/夜双态截图；E2E +5 条夜读断言 → **32→37**；pytest **329 不变**（印证后端零改动围栏 G5）。
- 注：本地截图里首页显示「暂无当日新鲜评分(0/0)」是本地未接 S3 缓存桶的环境假象，非 bug；
  后续阶段要截真数据需连生产或灌本地缓存。

## 2026-08-05 · S2 晨报决策首页（v4 三屏 loop cycle 2）

- **实现**：报头（`浪报 SURF DAILY · <GMT+8 日期>`）+ verdict 大标题（衬线、浪点名带下划线强调）
  + 覆盖计数 + 大评分/tag/headline/chips + 亚军行 + 本周走势 + 校准戳链 `/status`。
- **新增 `charts.sparkline(days)`**：折线 + 峰值放大标注 + 星期轴。提示用 SVG 原生 `<title>`
  而不是新造 tooltip 机制（Vue 版没有；单 HTML 版那套依赖全局 mousemove），顺带读屏可读。
- **走势的数据依赖**：`/api/recommend` 只给最佳那一天，7 天分要取该点 report。
  做成**渐进增强**：verdict 立即渲染，走势卡等 report 到了才出现——不让它拖慢首屏。
  `tag` 同理（recommend 无此字段，report 到位才显，不编造）。
- **两个自己引入的视觉缺陷，截图看出来后修掉**：
  ① 报头右侧链接被右上固定的 🌙/👤 压住（只露一个「国」字）→ 去掉报头链接、
     把「看全国浪况」放回三渐进入口（反而更贴原型①的三入口），并给报头留 96px 右padding；
  ② 走势峰值标注画在点上方 9px，峰值贴顶时被卡片裁掉 → SVG 顶部留白 12→20。
- 🔴 **一个真 bug（SWR 键即契约）**：我在首页用了 `report|<slug>` 存**裸 report**，
  而 `SpotPage` 同键存的是 `{ rep, hist, bias, hasLive }` 包装对象。首页先跑写入后，
  详情页读到裸对象 → `applyReport(v.rep)` 得 `undefined` → **详情页整页白**。
  E2E 抓到（详情三条连挂）。修法：首页改用独立键 `trend|<slug>`，并在 `swr.js` 顶部
  写下「键即契约：同键必须同形状」的约定 + 本次事故记录，防后来人再踩。
- **一条过时断言**：`ok('首页 标题', h1 含 '决策助手')` —— 改版后 h1 是 verdict、
  品牌移进报头，故更新为断言 `.brand` 含 `SURF DAILY`（不是放宽，是跟着规格改）。
- **验收**：pytest **329 不变**（后端零改动）· E2E **37→45** · 生产真数据截图
  `46-s2-home-real.png`（走势 6 点、峰值周一 8.65）。

## 2026-08-05 · S3 小白模式：倒计时 + 通勤倒推（v4 三屏 loop cycle 3）

- **逻辑抽成纯函数模块 `src/countdown.js`**（无 DOM、无 I/O）：`countdown()` / `departure()` /
  `windowHours()` / `gmt8Parts()`。这样才能用假时钟精确钉边界，而不是埋在组件里没法测。
  取 `day.windows[0]` 的**数字小时对**（如 `[[6,9]]`）而非解析 `window` 字符串；
  GMT+8 自算，不依赖运行环境时区。
- **三态双侧边界钉死**（Playwright `addInitScript` 冻结 Date，7 例）：
  起点前1秒→before · **起点整→during** · 终点前1秒→during · **终点整→after** ·
  深夜→after · 前一天(未来日)→before · 后一天(已过)→after。约定写在函数注释里。
- **通勤倒推**：窗口起点 − 车程 − 收拾装备 15min；± 可调存 `sf_commute_v1`；
  早于零点标「前一天」；仅小白模式（原型②专属，高手模式不显——E2E 有反向断言）。
- **可读性改进（真数据看出来的）**：最佳日常在数天后，倒计时原样会显示
  「114小时 18分 9秒」——数值对但没人这么读。≥48h 改「4天 18小时」。
- **两个测试脚手架的坑（产品代码没问题，但值得记）**：
  ① Playwright **后注册的 route 优先** —— 我把 `**/api/**` 兜底放最后，
     它把所有具体 mock 全盖住了，导致详情页压根没数据、误判成「倒计时不渲染」。兜底必须先注册。
  ② 假时钟注入的模板转义写成 `${'${JSON.stringify(iso)}'}`，生成的是字面量字符串 →
     `new Date` 得 Invalid Date → 边界例误判。已修并在注释里写明「别在模板里再套一层引号」。
  另外主 E2E 里假时钟必须对着 **`ranking[0]` 指定的显示日**设，不是 `days[0]`。
- **验收**：pytest **329 不变** · E2E **45→51** · 生产真数据截图 `47-s3-novice-real.png`。

## 2026-08-06 · S4 高手模式：风向罗盘 + m/ft 单位切换（v4 三屏 loop cycle 5）

- **`charts.compass(d)`**：扇区＝`SPOT_FACING`（原型写死 157° 只因它是单浪点 demo，
  生产按 `setFacing(report.spotFacingDeg)`）；三支箭＝06/12/18 时 `wdeg`，箭头指风的去向。
  缺某时刻不画那支、一支都没有则整个罗盘不渲染（不插值不假点）。提示用 SVG `<title>`。
  扇区色新增令牌 `--ch-facearc`，日/夜值逐字取原型 `T().face`。
- **S4.3 口径纪律**：罗盘内**零阈值代码**，全部走 `windKind`。E2E 用 facing 独立重算、
  逐支箭比对 `data-kind`（337°→off / 247°→cross / 157°→on），谁哪天在罗盘里另写阈值会立刻炸。

### 🔍 顺带查实的一个真问题：浪点朝向是全站单一常量
- `/api/report` 的 `spotFacingDeg` 对**全国每个浪点**都返回 **157°**，源头是
  `config/thresholds.yaml: spot_facing_deg: 157`（青岛山东头 SSE），后端风向评分用的是同一个值。
- 注册表里另有**逐点** `facing`（110/135/157/160 四种）与 `facing_calibrated` 标记，
  但生产 58 点 **`facing_calibrated` 全为 false**，且引擎并未采用它。
- 影响：离岸/向岸是产品的一等参数（domain-knowledge 明确），而它现在按一个全国常量判定。
  例：黄金海岸目录记 135°，分析仍按 157° → 差 22°，够把 cross↔offshore 判定推翻。
- **本轮没有偷偷换源**：前端若改用目录的 `facing`，就会和后端自己给出的「侧岸(ENE)」文案分叉，
  正是 S4.3 要防的口径漂移（且目录值本身也未校准，不是更可信）。
- 采取的动作：**把不确定性显式说出来** —— 度数后加「＊分析口径，未逐点校准
  （目录另记该点约 N°，引擎暂未采用）」，`facing_calibrated` 为真时该附注自动消失。
- 🔒 **留给用户的后端事项**：让引擎按注册表逐点 `facing` 计算；以及真正去校准这些朝向
  （README 早已标注「浪点朝向 facing 待校准」，本轮把它的实际影响量化了）。

- **`src/units.js` 作单位单一真源**：`unit` 用 Vue `ref` 而不是普通变量——图表函数在
  computed 里读它，切换后**自动重绘**，不必给每个图表函数加参数或手动触发。
  覆盖浪高轴刻度 / 柱顶数字 / tooltip（浪高+涌高）/ 昨日回看 `predict.height`。1m=3.281ft。
  `convertHeights` 的正则带前后守卫（避开 km、mm、m/s、min），且**只用于已核实语义是高度的字段**，
  绝不全局套后端叙事——那会篡改含义。
- **一次范围收敛**：我一度给 `pa`（五维逐项解释）和 `tideText` 写了换算，
  但这两个字段**原型和现有页面都不渲染** → 删掉，不留死代码。
- **三个我自己的失误（都在提交前修掉）**：模板里用了未定义的 `facingDeg`；
  引用成 `rep.value` 而响应式变量叫 `report`（会一直返回 null）；从 `../charts` 重复导入两行。
- **两条 E2E 断言是我写错**（产品无问题）：`.review` 是 `<details>` 默认折叠，
  `innerText` 只返回 summary → 要先点开；风质条 6 段是因为昨日回看也渲染了一个 ChartBox，不是 3。
- **验收**：pytest **329 不变** · E2E **51→61** · 真数据日夜双态截图（夜读＋英尺同时生效）。

## 2026-08-06 · S5.4 零上下文可用性评审的折回

派了一个**只看三张截图、不读任何本轮文档**的独立子代理扮演首次访问的冲浪爱好者。
它报出的最有价值一条，是我自己绝对看不出来的（我知道每个缩写什么意思）：

- 🔴 **首页 chip 直接显示原始枚举 `off`**。后端 `src/web/recommend.py:_key_factors` 把
  `day["dawnWind"]`（`off`/`cross`/`on`）原样塞进 `key_factors`。评审的第一反应是
  「off = 关闭 / 没风」——而它实际是**最好**的风况（离岸风梳面）。
  即：一个把最优条件误读成「没有」的标签。修法是**显示层翻译**，标签复用
  `charts.WIND_META`（与风质条、罗盘同一个来源，不新造口径）；认不出的 chip 原样透出不猜。
- 🔴 **`Tp9s` 术语裸奔** → 「峰周期 9s」。
- 🔴 **「窗口 18:00-21:00**到**」与通勤卡「17:00 出门 → 18:00 下水」语义冲突**。
  评审原话：按「到」理解会整整迟到一小时。引擎的 `window` 是最佳可冲时段（=在水里的时间），
  所以详情页改为明写「🕐 **下水窗口** 18:00-21:00」，与通勤卡的「出门/下水」对齐。
- 可发现性：夜读/登录图标只有 `aria-label`（鼠标用户看不到），补 `title`；
  通勤 ± 按钮补 `title`（评审说「车程 −45min ＋」容易被当成静态文字）。

### 🪞 顺带发现：E2E fixture 比现实漂亮
`vue_spa.mjs` 的 canned `key_factors` 原本写的是 `['0.9m浪', '离岸风', 'Tp7s']` ——
**已经人话化**的「离岸风」。而后端真实输出是 `'off'`。fixture 比现实漂亮，
于是「原始枚举直接示人」这个缺陷在 E2E 里**永远不可能暴露**。
已改为后端真值 `'off'` 并加注释「别再把 fixture 美化」。这条比那三个文案修复更值钱：
它是这类缺陷能长期存活的**机制**。

### 🔒 只能上报的（后端零改动围栏内改不了）
- `_key_factors` / `_headline` 应在**后端**就出人话（前端翻译只是兜底，别的客户端接进来会再踩）。
- `_headline` 的 `f"{win}到"` 措辞应改为「下水」或「在水里」。
- **`/api/recommend` 与 `/api/report` 对同一天同一浪点给出的分数精度不一致**
  （首页 8.7 vs 详情 8.65）。评审原话：「会让人犹豫是不是刷新后分变了」。

## 2026-08-06 · S5.3 原型对照 + 令牌/死代码审计的折回

第二路独立子代理做块级对照与机械审计，折回 3 项真问题：

- 🔴 **首页缺最佳窗口倒计时**：原型 `vHome` 里有 `${cdownEl(1,'⏳ ')}`，我 S2 时漏了。
  补的时候关键是**不新增请求**——走势卡已经取过该浪点的 report，倒计时复用同一份数据
  （同一份数据两个用途），并严格按 `best.day` 日期对齐找那天，不用「第一天」凑。
- 🔴 **选中日 halo 写死 `rgba(14,165,233,.2)`，夜读不跟随**。这是 S1「令牌化 126 处」漏掉的，
  而且是**夜读下真能看出来的缺陷**（夜里固定亮蓝刺眼）。新增 `--halo` 令牌日/夜两值，
  `getComputedStyle` 实测已随夜读切换。
- 🔴 `StatusPage` 阴影与 `--shadow` 重复、`App.vue` 遮罩硬编码 → 复用/新增 `--overlay`。
  现在 `src/` 下除 `style.css` **只剩 `LiveCam` 的 `#000`**（视频信箱底，产品例外）。

**我更正了审计的一处分类**：它把「首页横评缺浪高/晨风两列」判为 🔴（数据已存在），
但 `src/web/recommend.py:152-156` 的 `alternatives` 只给
`spot_slug/spot_name/day/week/score`，**没有** `hs`/`dawnWind` —— 要拿到得为每个亚军
再取一次 report（2 次额外请求）或扩后端。应为 🟡。

**有意未做**：`charts.lifecycleChart` 确认无生产调用者，但它不是本轮引入的，
删除属独立清理动作，留待确认无其他分支依赖后再做。小白页「分享」「加入日历」
原型有、纯前端可做，但不在本轮 ✅ 档清单内 —— 不趁机扩范围。
